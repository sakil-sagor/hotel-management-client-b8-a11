...
const FeaturedRooms = () => {
  ...
  useEffect(() => {
    let url = `https://ass8-b8.vercel.app/api/v1/rooms/all?sort=${sortByPrice}&page=${
      page + 1
    }&limit=${limit}`;
        ...
        <div className="full-width-all container m-auto text-white">
          <div>
            <div className="px-2 lg:px-0 pb-12 text-center">
              <h1 className={`text-4xl md:text-5xl text-sky-800  `}>
                Featured Rooms Design
              </h1>
              <br />
              <hr className="w-24 border-indigo-900 border-t-2 mx-auto" />
              <br />
              <h4 className={`text-xl w-full sm:w3/4 text-sky-800   `}>
                Amazing Rooms design added recently in our catalog
              </h4>
            </div>
          </div>
        </div>
