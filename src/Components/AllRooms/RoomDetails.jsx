...
const RoomDetails = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        let url = `https://ass8-b8.vercel.app/api/v1/rooms/all/${roomId}`;
    ...
    if (bookDate) {
      try {
        const response = await axiosSecure.put(
          `https://ass8-b8.vercel.app/api/v1/rooms/all/${roomId}`,
          bookingDate,
          {
            headers: {
                    ...
                    {singleRoom.discount > 0 ? (
                      <div>
                        ...
                        <p className="text-white font-semibold  ">
                          {singleRoom?.price -
                            (singleRoom?.price * singleRoom?.discount) / 100}
                          TK / <span className="text-xs">day</span>
                        </p>
                      </div>
                    ) : (
                      <p className="text-white font-semibold  ">
              ...
              <Tabs>
                ...
                <TabPanel>
                  <div className="text-gray-700 my-14">
                    <p className="mb-4">
                      {singleRoom?.description?.slice(0, 300)}
                    </p>
                    <p className="mb-4">
                      {singleRoom?.description?.slice(300, 500)}
                    </p>
                    <p className="mb-4">
                      {singleRoom?.description?.slice(400, 300000)}
                    </p>
                  </div>
                </TabPanel>
                <TabPanel>
